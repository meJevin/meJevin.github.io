...
for (let i = 0; i < expandableButtons.length; ++i) {
    ...
    event => {
        ...
        if (content.classList.contains("show"))
        {
            ...
            let rowValues =  document.defaultView.getComputedStyle(content, "").getPropertyValue("grid-template-rows").split(" ");
...
function OnResize() {
    for (let i = 0; i < expandableButtons.length; ++i)
    {
        ...
        if (content.classList.contains("show"))
        {
            ...
            console.log("Recomputed: " + content.style.maxHeight + " to " + finalContentHeight);
